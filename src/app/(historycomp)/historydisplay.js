        ...
        <ul className="px-1 text-white">
            ...
            {history.map((hist) => {
                return hist.convoArr.map((conv, index) => {
                    return (
                        <li className={`relative over history text-md history flex py-[0.5px] ml-[-0.25rem] px-1 rounded items-center justify-between cursor-pointer hover:bg-[#e7e9ee]/10 transition-colors duration-200 ease-in-out`} key={index}>
                            {!conv.isRename ?
